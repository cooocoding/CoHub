...
<template>
	<view class="uni-content">
		...
		<!-- 顶部文字 -->
		<text class="title">Please select login method</text>
		<!-- 快捷登录框 当url带参数时有效 -->
		<template v-if="['apple','weixin', 'weixinMobile'].includes(type)">
			<text class="tip">Your information will be obtained according to the authorization scope of the third-party account service platform</text>
			<view class="quickLogin">
				...
				<button v-else type="primary" open-type="getPhoneNumber" @getphonenumber="quickLogin"
					class="uni-btn">WeChat authorized mobile phone number login</button>
				<uni-id-pages-agreements scope="register" ref="agreements"></uni-id-pages-agreements>
			</view>
		</template>
		<template v-else>
			<text class="tip">Unregistered accounts will be automatically registered after passing verification</text>
			<view class="phone-box">
				...
				<uni-easyinput :focus="focusPhone" @blur="focusPhone = false" class="input-box" type="number"
					:inputBorder="false" v-model="phone" maxlength="11" placeholder="Please enter phone number" />
			</view>
			<uni-id-pages-agreements scope="register" ref="agreements"></uni-id-pages-agreements>
			<button class="uni-btn" type="primary" @click="toSmsPage">Get verification code</button>
		</template>
		<!-- 固定定位的快捷登录按钮 -->
